...
Unfortunately, importing `stdio.h` does not fix this problem, for two reasons. One, Terra did not understand inline functions on windows. They were ephemeral wisps, [vanishing like a mote of dust on the wind](https://github.com/zdevito/terra/issues/401) the moment a C module was optimized. A pull requested fixed this, but it can't fix a much more severe problem caused by the Windows header files invoking dark and unknowable runes that the linker must follow precisely, lest it cause a duplicate symbol error.
